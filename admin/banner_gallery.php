...
if(isset($_REQUEST[submit]) and $_REQUEST[submit]=="Upload")
{
	...
	{	//echo $_FILES['userfile']['name'][$i];
		if($_FILES['userfile']['name'][$i])
		{
			...
			$uploadfile = "../images/gallery/slider". $FileName;
...
if(isset($_REQUEST[act]) and $_REQUEST[act]=="d")
{
	...
	@unlink('../images/gallery/slider'.$_REQUEST[image_file]);
	$msg = 'Record deleted successfully';
	$msg_type="success";
}
          ...
          <div class="box table">
            ...
            <table width="100%" cellspacing="0" cellpadding="4" border="0" align="center" class="greyBorder">
                <thead>
                    ...
                    <tbody>
                    <?php 
                        $q = mysql_query("select * from tbl_gallery where type='slider' order by image_id DESC");
						...
						if ($totalRows > 0) 
						{
							while($row = mysql_fetch_array($q))
							{
								?>
								<tr>
									<td><img src="<?php echo '../images/gallery/slider'.$row['image_file']; ?>" alt="" width="150px"></td>
									<td><?php echo $row[image_url]; ?></td>
									<td style="padding:0px">
										<a href="banner_gallery.php?image_id=<?php echo $row[image_id]?>&image_file=<?php echo $row[image_file]?>&act=d" onclick="confirm('Are you sure you want to delete this image');">
